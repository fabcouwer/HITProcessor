package org.sikuli.design.color;
...
public class ColorAnalyzer {
	...
	static ImageExplainer logger = ImageExplainer
			.getExplainer(ColorAnalyzer.class);

	static public Map<NamedColor, Double> computeColorDistribution(
			BufferedImage input) {

		return computeColorDistribution(IplImage.createFrom(input),
				new StandardColors());
	}

	static private Map<NamedColor, Double> computeColorDistribution(
			IplImage input, NamedColors colors) {
	...
	static public IplImage computeMaskOfColor(IplImage input, Color color) {
		...
		CvScalar scalar = cvScalar(color.getBlue(), color.getGreen(),
				color.getRed(), 0);
	...
	static public int countNumberOfPixelsOfColor(IplImage input, Color color) {
		...
		IplImage diff = IplImage.createCompatible(input);
		// IplImage diff = IplImage.create(cvSize(input.width(),
		// input.height()),
		// 8, 3);
		CvScalar scalar = cvScalar(color.getBlue(), color.getGreen(),
				color.getRed(), 0);
		...
		IplImage r = IplImage.create(cvSize(input.width(), input.height()), 8,
				1);
		IplImage g = IplImage.create(cvSize(input.width(), input.height()), 8,
				1);
		IplImage b = IplImage.create(cvSize(input.width(), input.height()), 8,
				1);
		cvSplit(diff, b, g, r, null);
		diff.release();

		IplImage sumDiff = IplImage.create(
				cvSize(input.width(), input.height()), 8, 1);
		cvAdd(sumDiff, b, sumDiff, null);
		cvAdd(sumDiff, g, sumDiff, null);
		cvAdd(sumDiff, r, sumDiff, null);
		r.release();
	...
	static public CvScalar computeAverageHueSaturationValue(BufferedImage input) {
		...
		IplImage hsv = cvCreateImage(cvSize(color.width(), color.height()),
				IPL_DEPTH_8U, 3);
		...
		// Disabled this because it generated nullpointers
		// color.release();
		// hsv.release();
	...
	static public double computeColorfulness(BufferedImage input) {
		...
		CvMat rMat = new CvMat(r), gMat = new CvMat(g), bMat = new CvMat(b);

		CvMat alpha = CvMat.create(rows, cols), beta = CvMat.create(rows, cols);
		...
		CvScalar alphaMean = new CvScalar(), alphaStdDev = new CvScalar(), betaMean = new CvScalar(), betaStdDev = new CvScalar();
		...
		double metric = Math.sqrt(alphaStdDev.getVal(0) * alphaStdDev.getVal(0)
				...
				+ 0.3
				* Math.sqrt(alphaMean.getVal(0) * alphaMean.getVal(0)
	...
	static public double computeColorfulness2(BufferedImage input) {
		...
		IplImage luv = cvCreateImage(cvSize(color.width(), color.height()),
				IPL_DEPTH_8U, 3);
		...
		CvMat lMat = new CvMat(l), uMat = new CvMat(u), vMat = new CvMat(v);
		...
		CvScalar saturationMean = new CvScalar(), saturationStdDev = new CvScalar();
