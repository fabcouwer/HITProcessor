...
import org.sikuli.design.structure.BlockType;
	...
	private boolean isReasonableToBeText(Block node) {
		...
		Collections.sort(copyOfChildren, new Comparator<Block>() {
			public int compare(Block arg0, Block arg1) {
				return (int) (arg0.getBounds().getMaxX() - arg1.getBounds()
						.getMaxX());
			}
		});
		...
		for (int i = 0; i < copyOfChildren.size() - 1 && isAccepted; i++) {
			...
			double firstMinY = first.getBounds().getMinY(), firstMaxY = first
					.getBounds().getMaxY(), secondMinY = second.getBounds()
					.getMinY(), secondMaxY = second.getBounds().getMaxY();
