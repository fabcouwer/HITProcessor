...
public class VisualFeatureExtractor {
	...
	public static void computeXYFeatures(String xmlFolderName) {
		...
		try {
			...
			writer.append("filename,textArea,nonTextArea,maxLevel,"
			...
			for (File f : files) {
				...
				Block rootWithTextdetected = Block.loadFromXml(f
						.getAbsolutePath());
				...
				writer.append("webby_" + f.getName() + "," + textArea + ","
						+ nonTextLeavesArea + "," + maxLevel + "," + avgLevel
						+ "," + numOfLeaves + "," + numOf1stLevelNodes + ","
						+ numOf2ndLevelNodes + "," + numOf3rdLevelNodes + ","
						+ percentageLeafArea + "," + percentageLeafArea2 + ","
						+ numOfTextGroup + "," + numOfImageArea
				// numOfTextLeaves + "," +
